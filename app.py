...
try:
    ...
    df_plan = conn.read(worksheet="Plan_Maestro_34S")
    
    # Asegurar formato de fecha
    df_plan['Fecha_Inicio'] = pd.to_datetime(df_plan['Fecha_Inicio'])
    
    # 1. Determinar Semana Actual Basado en la Fecha Real (Hoy es Julio 2026)
    ...
    with col2:
        # Simulación de KPI de Consistencia (Se conectará dinámicamente al diario más adelante)
        st.markdown("""
            <div class="metric-box">
    ...
    with col4:
        # Semáforo de salud preventivo post-cirugía (L5-S1)
        st.markdown("""
            <div class="metric-box">
    ...
    st.write("Por favor, verifica que el enlace en 'Secrets' esté bien configurado y que la hoja tenga permisos de lectura para cualquier persona con el enlace.")
    st.exception(e)
